...
# CREATE GEOM INSTANCE TABLE
create_attr_table <- function(connectionDetails, schema, name) {
  conn <- connect(connectionDetails)
  DatabaseConnector::dbExecute(conn, paste0("CREATE SCHEMA IF NOT EXISTS ", schema, ";"))
  DatabaseConnector::dbExecute(conn, paste0("CREATE TABLE IF NOT EXISTS ", schema,
                                            ".\"attr_", name, "\" (
...
load_feature <- function(connectionDetails, feature_index_id){
  ...
  # TODO: add checks that attr and geom tables exist. call functions if not... for geom we need populated already... tbd...
  ...
  # TODO this only works on shape files.
  ...
  # TODO IF GEOM_X NOT LOADED AT THIS POINT, THIS IS ALL FOR NAUGHT. MAKE SURE THERE ARE CHECKS
  ...
  # import
  import_into_attr(connectionDetails, attr_to_ingest, attr_index_df)
  
  
  # TODO: use geom_id_map to populate 
