...
createDdl <- function(gaiaVersion){
  ...
  gaiaTableCsvLoc <- system.file(file.path("csv", paste0("gaia", gaiaVersion, "tableLevel.csv")), package = "GIS", mustWork = TRUE)
  gaiaFieldCsvLoc <- system.file(file.path("csv", paste0("gaia", gaiaVersion, "fieldLevel.csv")), package = "GIS", mustWork = TRUE)
  ...
  sql_result <- c()
  sql_result <- c(paste0("--@targetDialect CDM DDL Specification for OMOP Common Data Model ", gaiaVersion))
...
createPrimaryKeys <- function(gaiaVersion){
  ...
  # argument checks
  stopifnot(is.character(gaiaVersion), length(gaiaVersion) == 1, gaiaVersion %in% listSupportedVersions())
  
  cdmFieldCsvLoc <- system.file(file.path("csv", paste0("OMOP_CDMv", gaiaVersion, "_Field_Level.csv")), package = "CommonDataModel", mustWork = TRUE)
  cdmSpecs <- read.csv(cdmFieldCsvLoc, stringsAsFactors = FALSE)
  
  primaryKeys <- subset(cdmSpecs, isPrimaryKey == "Yes")
  pkFields <- primaryKeys$cdmFieldName
  
  sql_result <- c(paste0("--@targetDialect CDM Primary Key Constraints for OMOP Common Data Model ", gaiaVersion, "\n"))
...
createForeignKeys <- function(gaiaVersion){
  ...
  sql_result <- c(paste0("--@targetDialect CDM Foreign Key Constraints for OMOP Common Data Model ", gaiaVersion, "\n"))
